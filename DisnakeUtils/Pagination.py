...
class AutoEmbedPaginator(object):
    ...
    timeout: Optional[`int`]
        Number in seconds before timeout. Defaults to 60 seconds.
    auto_footer: Optional[`bool`]
        Display page number below footer. Defaults to False.
    remove_reactions: Optional[`bool`]
        Removes reaction after timeout. Defaults to True.
    ...
    --------
    auto_embed:
        Returns an embed with all 5 controls.
    """
    ...
    async def run(self, embeds, send_to=None):
        ...
        -----------
        embeds: :class:`list`
            The list of embeds to run.

        send_to: Optional[`disnake.Member`]
            The member to send to. Defaults to the channel where you executed the command.
        """
        if not send_to:
            send_to = self.ctx
        ...
        if not self.embeds:
            self.embeds = embeds
        if self.auto_footer:
            self.embeds[0].set_footer(text=f'Page {self.current_page+1} / {len(self.embeds)}')
        if len(self.embeds) == 1: # we don't make the reactions appear if the embeds provided is equal to 1.
            return await send_to.send(embed=self.embeds[0])
        await send_to.send(embed=self.embeds[0])
        msg = await send_to.original_message()
            ...
            if str(reaction.emoji) == self.control_emojis[0]:
                ...
                if self.remove_reactions:
                    try:
                        ...
                        pass
                if self.auto_footer:
                    self.embeds[len(self.embeds)-1].set_footer(text=f'Page {self.current_page+1} / {len(self.embeds)}')
...
class CustomEmbedPaginator(object):
    ...
    inter: Union[:class:`disnake.ApplicationCommandInteraction`, :class:`disnake.Message`, :class:`disnake.ext.commands.Context]`
        Represents the interaction itself.
    emojis: :class:`disnake.PartialEmoji`
        Customizes the emoji output.
    timeout: Optional[`int`]
        Number in seconds before timeout. Defaults to 60 seconds.
    auto_footer: Optional[`bool`]
        Display page number below footer. Defaults to False.
    ...
    --------
    auto_embed:
        Returns an embed with certain controls.
    """
    def __init__(self, ctx, **kwargs):
        self.embeds = None
        self.ctx = ctx
    ...
    def add_reaction(self, emoji, command):
        """Adds a reaction to the reaction list.
        
        Parameters
        -----------
        emoji: :class:`disnake.PartialEmoji`
            The emoji to display on reactions.

        command: :class:`str`
            The type of reaction. It should be one of the following: `first, next, delete, back, last, clear/lock`
        """
        self.control_emojis.append(emoji)
        self.control_commands.append(command)
    def insert_reaction(self, index, emoji, command):
        """Inserts a reaction to the reaction list.
        
        Parameters
        -----------
        index: :class:`int`
            The index which inserts the new reaction.
    ...
    def remove_reaction_at(self, index):
        """Removes existing reaction to a specified index.
        
        Parameters
        -----------
        index: :class:`int`
            The index to remove the reaction.
        """
    ...
    async def run(self, embeds, send_to=None):
        ...
        send_to: :class:`disnake.Member` [Optional]
            The member to send to. Defaults to the channel where you executed the command.
        """
        self.embeds = embeds
        if not send_to:
