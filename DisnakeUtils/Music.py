...
try:
    import youtube_dl
    from disnake import PCMVolumeTransformer, FFmpegPCMAudio
    has_voice = True
except ImportError:
    has_voice = False
...
async def get_video_data(url, search, bettersearch, loop):
    if not has_voice:
        raise RuntimeError("DisnakeUtils[voice] install needed in order to use voice")
...
def check_queue(ctx, opts, music, after, on_play, loop):
    ...
    if not song.is_looping:
        ...
        if len(music.queue[ctx.guild.id]) > 0:
            source = PCMVolumeTransformer(FFmpegPCMAudio(music.queue[ctx.guild.id][0].source, **opts))
...
class Music(object):
    ...
    def create_player(self, ctx, **kwargs):
        ...
        Parameters
        -----------
        inter: Union[:class:`disnake.ApplicationCommandInteraction`, :class:`disnake.Message`, :class:`disnake.ext.commands.Context]`
            Represents the interaction itself.

        ffmpeg_error_betterfix: :class:`bool`
            Provides reconnection improvements upon playing a song.
        """
    ...
    def get_player(self, **kwargs):
        ...
        Parameters
        -----------
        guild_id: :class:`disnake.Guild`
            The ID of the guild to initialize.

        channel_id: Optional[`disnake.VoiceChannel`]
            The ID of the channel currently joined.
        """
...
class MusicPlayer(object):
    ...
    async def queue(self, url, search=False, bettersearch=False):
        """Queues a ``Song`` object if the bot is playing another song.
        ...
        search: Optional[`bool`]
            Whether to enable search query or not. Defaults to False.

        bettersearch: Optional[`bool`]
            Enables better searching method. Defaults to False.

        Returns
        --------
        song: :class:`Song`
            A ``Song`` object ready to play.
        """
        ...
        return song
    async def play(self):
        """Plays the queried song."""
        source = PCMVolumeTransformer(FFmpegPCMAudio(self.music.queue[self.ctx.guild.id][0].source, **self.ffmpeg_opts))
    ...
    async def skip(self, force=False):
        ...
        -----------
        force: Optional[`bool`]
            Immediately skips the music. Defaults to False.
        """
        if len(self.music.queue[self.ctx.guild.id]) == 0:
            ...
            try:
                ...
                return old
    async def stop(self):
        """Stops the queue from playing."""
        try:
            self.music.queue[self.ctx.guild.id] = []
            self.voice.stop()
            self.music.players.remove(self)
        except:
            raise NotPlaying("Cannot stop player because nothing is being played")
    ...
    def current_queue(self):
        ...
        try:
            ...
            raise EmptyQueue("Cannot display non-existent list of songs")
    def now_playing(self):
        """Displays current music playing.
    ...
    async def change_volume(self, vol):
        ...
        try:
            ...
            raise NotPlaying("Cannot change volume because nothing is being played")
        if self.on_volume_change_func:
            await self.on_volume_change_func(self.ctx, song, vol)
        return (song, vol)
    async def remove_from_queue(self, index):
        """Removes a selected song from queue.
        
        Parameters
        ----------
        index: :class:`int`
            The song index to remove.
        """
        if index == 0:
            try:
                song = self.music.queue[self.ctx.guild.id][0]
            except:
                raise NotPlaying("Cannot remove non-existent song")
